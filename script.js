// criar variável modalKey sera global
let modalKey = 0

// variavel que irá controlar a quantidade inicial de pizzas
let quantPizzas = 1
...
const preencheDadosDasPizzas = (pizzaItem, item, index) => {
    // identificar qual elemento foi clicado
...
const pegarKey = (e) => {
    // .closest retorna o elemento mais próximo que tem a class que foi indicada
...
const escolherTamanhoPreco = (key) => {
    ...
    selecionaTodos('.pizzaInfo--size').forEach((size, sizeIndex) => {
        size.addEventListener('click', (e) => {
            // clicou em um item, tirar a seleção dos outros e marca o que foi clicada
            // tirar a seleção de tamanho atual e selecionar o tamanho grande
            seleciona('.pizzaInfo--size.selected').classList.remove('selected')
            // marcar a clicada, ao inves de usar e.target use size, pois ele é o item dentro do loop
...
const adicionarNoCarrinho = () => {
    seleciona('.pizzaInfo--addButton').addEventListener('click', () => {
	    ...
	    // concatena as duas informacoes separadas por um símbolo
...
const atualizarCarrinho = () => {
	...
	if(cart.length > 0) {
		...
		// zerar meu .cart para nao fazer inserções duplicadas
		seleciona('.cart').innerHTML = ''

        // lembrar de criar variaveis antes do for
		let subtotal = 0
		let desconto = 0
		let total    = 0
		...
		for(let i in cart) {
			// usar o find para pegar o item por id
...
// MAPEAR o pizzaJson para gerar a lista de pizzas
